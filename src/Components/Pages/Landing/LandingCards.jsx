...
const videoCache =
  globalThis.__landingVideoCache ||
  (globalThis.__landingVideoCache = new Map());
...
export const LandingCards = ({ product, showNav, squareCard, index }) => {
  ...
  const displayName =
    i18n.language === "fr" && product.french_name
      ? product.french_name
      : product.name;
  ...
  const handleFavorite = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/user/add/favorite/bundle/${product.id}`,
        payload,
      );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!hasSignaledReady.current) {
        ...
        window.dispatchEvent(new Event("biogance-home-ready"));
      }
